...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		v1.add("aaa");
		v1.add(111);
		v1.add(new Integer(123));  //Auto boxing?
		v1.add('a');  //character
		v1.add(true);
		v1.add(3.14);
		//rapper? class??
		System.out.println("현재크기 : " + v1.size());   // v1.add들 6개 
		...
		// add(index, data) => vector의 index번째에 '데이터'를 끼워 넣는다
		//					=> index는 0부터 시작한다
		
		v1.add(1, "kkk");
		System.out.println("v1 =>" + v1.toString());
		
		//set(index, data) => vector의 index번째의 값을 주어진 '데이터'로 덮어쓴다 (반환값 : 원래의 데이터)
		String temp = (String)v1.set(0, "zzz");  //기존데이터 temp에 따로 빼줌
		System.out.println("set명령 후 v1 => " + v1);
		System.out.println("원래의 데이터 : " + temp);
		
		// remove(index) => vector의 index번째 자료를 삭제한다
		//				 => 자료가 삭제되면 index번째 다음번쨰 이후의 데이터들이 앞으로 장동으로 당겨져서 채워진다
								
		// remove(삭제할 데이터) => '삭제할 데이터'를 찾아 삭제한다
		//					 => 만약 '삭제할 데이터'가 여러개면 앞에서부터 삭제함
		//					 => 삭제할 데이터가 '정수형'이거나 'char'일 경우에는 삭제할 데이터를 객체로 변환하여 사용해야 함
		v1.remove(0);
		...
		v1.remove(new Integer(123));  //remove => index num과 값 두가지 읽음 (123) or (123번째 index)
		System.out.println("삭제후 : " + v1);
		System.out.println();
		...
		Vector<Integer> v3 = new Vector<Integer>();  //Integer절만 저장가능
		
		v2.add("안녕하슈");
		//v2.add(123); //오류 : 다른 종류의 데이터를 추가할 수 없슴
		...
		//					Collertion객체가 가지고 있는 데이터를 모두 삭제한다
		v2.removeAll(v4);
		...
		/*
		 ...
		 => 반복할 때 마다 '배열변수나 Collection계열 변수'의 데이터를 하나씩 꺼내와
		 	'변수명'에 저장한 후 '처리할 내용들'을 처리한다
		 */
		for(String s : v2) {    //순회하면서 v2에 있는거 하나씩 꺼내옴 ':' s에 담겨짐 ('값'을 읽어, 인덱스는 알수 없슴)
		...
		System.out.println("------------------------------------");
			...
			v.ensureCapacity(5); //현재 용량이 최고용량보다 작다면...
								 // 신규용량 = 현재용량 *2
			...
			System.out.println("==== After setSize(7) ====");  //6 * 2 == capa 12가 나옴 (7보다 크니까~)
			print(v);
	...
	public static void print(Vector v) {
		...
		System.out.println("capacity :  " + v.capacity());  //data && size (capa는 그대로 12)
	}
