...
public class T12_Equals_hashCode {
/*
 ...
 * 	HashSet, HashMap, Hashtable에서는 객체가 같은지 여부는 데이터를 추가(add)할 때 검사한다
 * 
 * 	- equals()메서드는 두 객체의 내용(값)이 같은지 비교하는 메서드이고,
 * 	- hashCode()메서드는 두 객체가 같은 객체인지를 비교하는 메서드이다
 * 
 * 	- equlas()메서드와 hashCode()메서드에 관련된 규칙
 * 	1 두 객체가 같은면 반드시 같은 hashCode를 가져야 한다
 * 	2 두 객체가 같으면 equals()메서드를 호출했을 때 true를 반환해야 한다
 * 		즉, 객체 a, b가 같으면 a.equals(b)와 b.equlas(a) 둘다 true여야 한다
 * 	3 두 객체의 hashcode가 같다고 해서 두 객체가 반드시 같은 객체는 아니다
 * 	   하지만 두 객체가 같은면 반드시 hashCode가 같아야 한다
 ...
 * 
 * 	- hashCode()메서드에서 사용하는 '해시 알고리즘'에서 서로 다른 객체에 대해 같은 hashcode값을 만들 수 있다
 * 	   그래서 객체가 같지 않더라도 hashcode가 같을 수 있다
 */
	public static void main(String[] args) {
		
		Person p1 = new Person(1, "ash");
		Person p2 = new Person(1, "ash");
		Person p3 = new Person(1, "lux");
		
		System.out.println("p1.equals(p2) : " + p1.equals(p2));  //컬렉션에 데이터는 넣는 시점에 중복체크 실행
		System.out.println("p1 == p2 : " + (p1 == p2));  //==은 주소값 비교
		//hashcode와 equals는 항상 같이 따라다님(중복체크)
...
*/		return (name + id).hashCode();  //hashCode추출
	}
	@Override
