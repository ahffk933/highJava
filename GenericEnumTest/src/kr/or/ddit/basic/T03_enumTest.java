 ...
 * 
 * 열거형 선언하는 방법
 * 	enum열거형 이름 {상수값1, 상수값2, ..., 상수값n};
 * 
 */
public class T03_enumTest {
	//City 열거형 객체 선언(기본값을 이용하는 열거형)
	public enum City {서울, 부산, 대구, 대전, 광주};  //상수값 (city type -> 서울, 대전, 대구.....)
	
	//데이터 값을 임의로 지정한 열거형 객체 선언
	//데이터 값을 정해줄 경우에는 생성자를 만들어서 괄호 속의 값이 변수에 저장되도록 해야한다
	public enum Season{  //시즌이라는 이넘타입  {4개의 상수 -> 봄,여름,가을,겨울}
		...
		private Season(String data) {  //data안에 "3월부터5월까지" ...가 들어감
			str = data;
		}
	...
	public static void main(String[] args) {
		 ...
		 * 2 ordinal()(순서) => 열거형 상수가 정의된 순소값을 반환한다(기본적으로 0 부터 시작)
		 * 3 valueOf("열거형 상수이름"); => 지정된 열거형에서 '열거형상수이름'과 일치하는 열거형 상수를 반환한다
		 */
		...
		//열거형 객체변수에 값 저장하기
		myCity2 = City.서울;
		myCity1 = City.valueOf("서울"); //City enum에서 '서울'인 객체반환
		...
		System.out.println("get method => " + ss.getStr());
		System.out.println("-----------------------------------------");
		
		//열거형이름.values() => 데이터를 배열로 가져온다
		Season[] enumArr = Season.values();   //프라이빗이라 다른데서 호출 불가능
