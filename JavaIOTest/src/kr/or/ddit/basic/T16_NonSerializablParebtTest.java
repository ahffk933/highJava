...
/*
 	부모클래스가 Serializable인터페이스를 구현하고 있지 않을 걍우 부모객체의 필드값 처리 방법
 	
 		1 부모클래스가 Serializable인터페이스를 구현하도록 해야함
 		2 자식 클래스에 writeObject()와 readObject()메서드를 이용하여 부모객체의 필드값을 처리할 수 있도록 직접 구현한다
 */
public class T16_NonSerializablParebtTest {
	public static void main(String[] args) throws IOException, Exception{
		...
		Child child2 = (Child) ois.readObject();  //열직렬화
...
class Child extends Parent implements Serializable{
	...
	private void writeObject(ObjectOutputStream out) throws IOException{
		//objectOutputStream 객체의 메서드를 이용하여 부모객채 필드값 처리
