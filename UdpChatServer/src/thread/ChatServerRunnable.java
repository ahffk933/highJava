...
public class ChatServerRunnable implements Runnable{
	...
	private Map<String, ClientVO> clientMap;    //중요!
	...
	public ChatServerRunnable(ObservableList<String> clientList, Map<String, ClientVO> clientMap){
		try {
			...
			//0.5초가 지나면 '타임아웃 익셉션'을 발생시키고 넘어가게 됨
	...
	public void run(){
		while(isServerOn){  //상태변수
			...
			try {
				...
				if(!isExist){ // 목록에 존재하지 않는 사용자라면 false, 있으면 true
					Platform.runLater(new Runnable() {  //화면 ui관련 작업은 javaFX tread만 가능?
					...
					ClientVO vo = new ClientVO(address.getHostAddress(),   //IP주소
												port, 
												new String(inPacket.getData()).trim() // 대화명(닉넴)에서 불필요한 공백 제거.
												);
					...
					Iterator<String> it = clientMap.keySet().iterator();  //맵 뒤져봄
					
					ClientVO senderVO =  clientMap.get(address.getHostName());  //vo객체를 꺼내옴
